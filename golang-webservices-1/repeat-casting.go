...
func Buy(p Payer, price int) {
    switch p.(type) {
    case *Wallet:
        fmt.Println("Would you like to pay by cash?")
    case *Card:
        // This causes panic:
        // fmt.Println(p.CardholderName)
        plasticCard, ok := p.(*Card)
        if ! ok {
            fmt.Println("Casting to type *Card failed")
        }
        fmt.Println(plasticCard.CardholderName, "please insert the card")
    default:
        fmt.Println("Unreckognized payment method")
    }
    err := p.Pay(price)
    if err != nil {
